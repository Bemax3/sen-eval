...
<template>
	<div class="sticky top-0 z-40 flex h-16 shrink-0 items-center gap-x-4 border-b border-gray-200 bg-white px-4 shadow-sm sm:gap-x-6 sm:px-6 lg:px-8">
		<button class="-m-2.5 p-2.5 text-gray-700 lg:hidden" type="button" @click="$emit('openSidebar')">
			...
			<div class="flex items-center gap-x-4 lg:gap-x-6">
				...
				<Menu as="div" class="relative">
					<MenuButton class="-m-1.5 flex items-center p-1.5">
                        ...
                        <span aria-hidden="true" class="ml-4 text-sm font-semibold leading-6 text-gray-900">{{ user.user_display_name }}</span>
					...
					<transition enter-active-class="transition ease-out duration-100" enter-from-class="transform opacity-0 scale-95"
						...
						<MenuItems
							...
							<MenuItem v-for="item in userNavigation" :key="item.name" v-slot="{ active }">
								<Link :class="[active ? 'bg-cyan-600  text-white' : 'text-gray-900', 'block px-3 py-1 text-sm leading-6 ']" :href="item.href"
								      as="span">
									{{ item.name }}
								</Link>
							</MenuItem>
							<MenuItem v-slot="{active}">
								<Link :class="[active ? 'bg-cyan-600  text-white' : 'text-gray-900', 'block px-3 py-1 text-sm leading-6 ']"
								      :href="route('logout')"
								      as="span" method="post">
									Déconnexion
								</Link>
