                                    ...
                                    <li>
                                        <ul role="list" class="-mx-2 space-y-1">
                                            <li v-for="item in navigation" :key="item.name">
                                                <Link v-if="!item.children" :href="item.href" :class="[item.current ? 'bg-cyan-500 text-white' : 'text-white hover:text-white hover:bg-cyan-500', 'group flex gap-x-3 rounded-md p-2 text-base leading-6 font-semibold']">
	                                            ...
	                                            <Disclosure as="div" v-else v-slot="{ open }">
		                                            <DisclosureButton :class="[item.current ? 'bg-cyan-500 text-white' : 'text-white hover:text-white hover:bg-cyan-500', 'flex items-center w-full text-left rounded-md p-2 gap-x-3 text-base leading-6 font-semibold text-white']">
			                                            <component :is="item.icon" :class="[item.current ? 'text-white' : 'text-white group-hover:text-white', 'h-6 w-6 shrink-0']" aria-hidden="true" />
			                                            {{ item.name }}
		                                            ...
		                                            <DisclosurePanel as="ul" class="mt-1 px-2">
			                                            <li v-for="subItem in item.children" :key="subItem.name">
				                                            <!-- 44px -->
				                                            <DisclosureButton as="a" :href="subItem.href" :class="[subItem.current ? 'bg-cyan-500 text-white' : 'text-white hover:text-white hover:bg-cyan-500', 'block rounded-md py-2 pr-2 pl-9 font-semibold text-base leading-6 text-gray-700']">{{ subItem.name }}</DisclosureButton>
			                                            </li>
		                                            </DisclosurePanel>
	                                            </Disclosure>
                                            </li>
                            ...
                            <li v-for="item in navigation" :key="item.name">
	                            ...
	                            <Disclosure as="div" v-else v-slot="{ open }">
		                            ...
		                            <DisclosurePanel as="ul" class="mt-1 px-2 space-y-1">
			                            <li v-for="subItem in item.children" :key="subItem.name">
				                            <!-- 44px -->
				                            <DisclosureButton as="a" :href="subItem.href" :class="[subItem.current ? 'bg-cyan-500 text-white' : 'text-white hover:text-white hover:bg-cyan-500', 'block rounded-md py-2 pr-2 pl-9 text-base font-semibold leading-6 text-gray-700']">{{ subItem.name }}</DisclosureButton>
			                            </li>
		                            </DisclosurePanel>
	                            </Disclosure>
                            </li>
