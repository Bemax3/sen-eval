...
<template>
    ...
    <h2 class="mt-2 text-sm font-semibold leading-6 text-gray-900 dark:text-white">Activité</h2>
            ...
            <template v-if="activityItem.type === 'commented'">
                ...
                <div class="flex-auto rounded-md p-3 ring-1 ring-inset ring-gray-200 dark:ring-gray-600">
                    <div class="flex justify-between gap-x-4">
                        <div class="py-0.5 text-xs leading-5 text-gray-500 dark:text-gray-100">
                            <span class="font-medium text-gray-900 dark:text-white">{{ activityItem.person.name }}</span> a commenté
                            <br><span class="font-medium text-gray-900 dark:text-white">Taux de réalisation</span>
                            <span class="ml-1 text-sm font-semibold text-cyan-600"> {{ activityItem.rate }} % </span>
                        </div>
                        <time :datetime="activityItem.dateTime" class="flex-auto text-right py-0.5 text-xs leading-5 text-gray-500 dark:text-gray-100">
                            {{ activityItem.date }}
                        </time>
                    </div>
                    <p class="text-sm leading-6 text-gray-500 dark:text-gray-100">{{ activityItem.comment }}</p>
                </div>
            </template>
            <template v-else>
                <div class="relative flex h-6 w-6 flex-none items-center justify-center bg-white dark:bg-grayish">
                ...
                <div class="flex-auto  p-3">
                    <div class="flex justify-between gap-x-4">
                        <div class="py-0.5 text-xs leading-5 text-gray-500 dark:text-gray-100">
                            <span class="font-medium text-gray-900 dark:text-white">{{ activityItem.person.name }}</span> {{ activityItem.type }} l'objectif.
                            <br><span class="font-medium text-gray-900 dark:text-white">Taux de réalisation</span>
                            <span class="ml-1 text-sm font-semibold text-cyan-600"> {{ activityItem.rate }} % </span>
                        </div>
                        <time :datetime="activityItem.dateTime" class="flex-auto text-right py-0.5 text-xs leading-5 text-gray-500 dark:text-gray-100">
