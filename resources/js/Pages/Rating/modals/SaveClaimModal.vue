...
const o = ref(false)
let form;
const setupForm = (claim) => {
    form = useForm(claim === undefined ? {
        claim_type_id: hasData(props.types) ? props.types[0].claim_type_id : null,
        rating_claim_comment: ''
    } : {
        rating_claim_id: claim.rating_claim_id,
        claim_type_id: claim.claim_type_id,
        rating_claim_comment: claim.rating_claim_comment || ''
    })

}
const submit = () => {
    if (form.rating_claim_id)
        form.put(route('rating-claims.update', {rating: props.rating.rating_id, rating_claim: form.rating_claim_id}), {
            onSuccess: params => {
                o.value = false;
                emit('closeModal');
                setupForm()
