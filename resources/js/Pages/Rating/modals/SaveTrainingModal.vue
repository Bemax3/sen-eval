...
const setupForm = (training) => {
    const starter = training === undefined ? {
        comment: '',
        training_type_id: hasData(props.types) ? props.types[0].training_type_id : null
    } : {
        rating_training_id: training.rating_training_id,
        comment: training.comment || '',
        training_type_id: training.training_type_id
    };
    form = useForm(props.isEvaluated ? {
            ...starter,
...
const submit = () => {
    if (form.rating_training_id) form.put(route('rating-trainings.update', {
        rating: props.rating.rating_id,
        rating_training: form.rating_training_id
    }), {
        onSuccess: params => {
            o.value = false;
            emit('closeModal');
    ...
    else form.post(route('rating-trainings.store', {rating: props.rating.rating_id}), {
        onSuccess: params => {
            o.value = false;
            emit('closeModal');
            setupForm()
                    ...
                    <TransitionChild as="template" enter="ease-out duration-300" enter-from="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95"
                        ...
                        <DialogPanel
                            ...
                            <form v-if="!isValidator && !rating.rating_is_validated"
                                ...
                                <h3 class="text-base font-semibold leading-6 text-gray-900 dark:text-white">Demander une formation</h3>
                                                        ...
                                                        <ListboxOptions v-if="hasData(types)"
                                                            ...
                                                            <ListboxOption v-for="type in types" :key="type.training_type_id" v-slot="{ active, selected }"
                                                                ...
                                                                <li :class="[active ? 'bg-cyan-600  text-white' : 'text-gray-900 dark:text-white', 'relative cursor-default select-none py-2 pl-3 pr-9']">
                                                                    <span
                                                                        :class="[selected ? 'font-semibold' : 'font-normal', 'block truncate']">{{ type.training_type_name
                                                                        }}</span>
                                                                    <span v-if="selected"
