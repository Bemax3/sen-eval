...
<template>
    <AuthenticatedLayout>
        <Head title="Modifier Organisation"/>
        <div class="px-4 sm:px-6 lg:px-8">
            <Breadcrumbs :pages="pages"/>
            <div class="sm:flex sm:items-center">
                <div class="sm:flex-auto">
                    <h1 class="text-2xl font-semibold leading-6 text-gray-900 dark:text-white">{{ title }}</h1>
                    <p class="mt-2 text-sm text-gray-700 dark:text-white">Ajouter ou modifier une organisation.</p>
                </div>
            </div>
                        ...
                        <div class="grid max-w-2xl grid-cols-1 gap-x-6 gap-y-8 sm:grid-cols-6">
                            ...
                            <div class="sm:col-span-4">
                                <Combobox v-model="form.parent_id" as="div">
                                    <ComboboxLabel class="block text-sm font-medium leading-6 text-gray-900 dark:text-white">Organisation Parent</ComboboxLabel>
                                            ...
                                            <ComboboxOption v-for="org in filteredOrg" :key="org.org_id" v-slot="{ active, selected }" :value="org.org_id" as="template">
                                                <li :class="['relative cursor-default select-none py-2 pl-3 pr-9', active ? 'bg-cyan-600  text-white' : 'text-gray-900 dark:text-white']">
                                                    <div class="flex">
                                                        ...
                                                        <span
                                                            :class="['ml-2 truncate text-gray-500 dark:text-gray-100', active ? 'text-cyan-200' : 'text-gray-500 dark:text-gray-100']">
