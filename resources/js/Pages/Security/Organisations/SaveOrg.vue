...
const query = ref('')
const selectedOrg = ref(form.parent_id)
                ...
                <form class="bg-white shadow-sm ring-1 ring-gray-900/5 sm:rounded-xl md:col-span-2" @submit.prevent="submit">
                    <div class="px-4 py-6 sm:p-8">
                        <div class="grid max-w-2xl grid-cols-1 gap-x-6 gap-y-8 sm:grid-cols-6">
                            ...
                            <div class="sm:col-span-4">
                                <Combobox as="div" v-model="form.parent_id">
                                    <ComboboxLabel class="block text-sm font-medium leading-6 text-gray-900">Organisation Parent</ComboboxLabel>
                                    <div class="relative mt-2">
                                        <ComboboxInput class="w-full rounded-md border-0 bg-white py-1.5 pl-3 pr-12 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 focus:ring-2 focus:ring-inset focus:ring-cyan-600 sm:text-sm sm:leading-6" @change="query = $event.target.value" :display-value="(id) => orgs.filter(x => x.org_id === id)[0]?.org_name" />
                                        <ComboboxButton class="absolute inset-y-0 right-0 flex items-center rounded-r-md px-2 focus:outline-none">
                                            <ChevronUpDownIcon class="h-5 w-5 text-gray-400" aria-hidden="true" />
                                        </ComboboxButton>
                                        <ComboboxOptions v-if="filteredOrg.length > 0" class="absolute z-10 mt-1 max-h-60 w-full overflow-auto rounded-md bg-white py-1 text-base shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none sm:text-sm">
                                            <ComboboxOption v-for="org in filteredOrg" :key="org.org_id" :value="org.org_id" as="template" v-slot="{ active, selected }">
